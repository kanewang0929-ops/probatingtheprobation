...
| 节奏字幕 | 文案、字体（毛笔／宋体）、配色（白字／黑字）、字号、画面上的位置、出现与消失的秒数、顺序、发布状态 |
...
字体有毛笔（Ma Shan Zheng）和宋体两种，配色有白字深底和黑字浅底两套——底板会跟着文字颜色一起翻，保证在深色画面和漫画白底上都读得清。

「出现／消失」两个秒数对应主视频的进度，决定这一句停留多久；两条区间重叠就会同时出现。

一个取舍：文字框以定位点为中心，能用的宽度是定位点到最近一侧边缘距离的两倍。
这样拖到画面边上也不会被裁掉，代价是越靠边可用宽度越窄、文字会折更多行。居中时和原来一样是 92vw／1100px。
...
**预览和发布都会自动先保存当前改动**，不会因为忘记点「保存草稿」而丢失。
「保存草稿」按钮仍然保留，用于改到一半先存着、之后再继续。
预览会显示未发布的条目，正式页面不会。
