...
配色有白字深底和黑字浅底两套，底板会跟着文字颜色一起翻，保证在深色影棚和漫画白底上都读得清。

排版可选横排或竖排。竖排是 `writing-mode: vertical-rl`，从上往下、从右往左，汉字保持正立。
竖排的列长按字数算（每字约一个 em），超过 86vh 会折成多列——竖排时文字的行进方向变成纵向，
`width: max-content` 管不到它，不显式给高度的话列长会退化成「从定位点到视口底部的剩余空间」，
字幕放得越靠下字被挤得越短。
