...
export function renderSubpage(page, opts = {}) {
  ...
  const bg = page.background
    ...
    : '';
  const prev = page.n > 1 ? `<a class="nav__link" href="${preview ? '/preview' : ''}/p/${page.n - 1}">← 上一篇</a>` : '<span></span>';
  const next = page.n < page.total ? `<a class="nav__link" href="${preview ? '/preview' : ''}/p/${page.n + 1}">下一篇 →</a>` : '<span></span>';
...
.top .n{font-family:var(--brush);font-size:26px;color:var(--rim);line-height:1}
...
${preview ? '<div class="draft">草稿预览 · 这一页尚未发布或仅你可见</div>' : ''}
  <div class="top">
    <span class="n">${page.n}</span>
    <a href="${preview ? '/preview' : '/'}">← 回到${esc(brand || '首页')}</a>
