...
export const LIMITS = {
  ...
  noteBody:   { min: 1, max: 200 },
  noteCue:    { min: 1, max: 30 },
...
export function getPage(content, n, { onlyPublished }) {
  ...
  /* 上一篇／下一篇要在「真正存在且进得去」的页之间跳。
     编号是按全部步骤连续排的，中间可能大段没建页或没发布，
     按 n±1 算会直接指到 404 上。*/
  const reachable = pages
    .filter(e => {
      const pg = value.pages?.[e.stepId];
      if (!pg) return false;
      return onlyPublished ? (e.bubblePublished && pg.published) : true;
    })
    .map(e => e.n);
  const i = reachable.indexOf(n);

  return {
    ...page, n,
    total: pages.length,
    prevN: i > 0 ? reachable[i - 1] : null,
    nextN: i >= 0 && i < reachable.length - 1 ? reachable[i + 1] : null,
    position: i + 1,          // 在可访问的页里排第几
    count: reachable.length,  // 可访问的页一共几篇
    bubbleName: entry.bubbleName, stepText: entry.stepText
  };
}
