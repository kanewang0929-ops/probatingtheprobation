...
console.log('泡泡数:', await card(p, '泡泡').locator('.item').count());
console.log('字幕数:', await card(p, '节奏字幕').locator('.item').count());
...
// 改一个泡泡名称 → 应出现「有改动待保存」
const nameInput = card(p, '泡泡').locator('.item').first().locator('input[type=text]').first();
...
console.log('超长计数器标红:', await card(p, '泡泡').locator('.item').first().locator('.count').first().evaluate(n => n.classList.contains('over')));
await p.click('.bar .btn:has-text("保存草稿")');
...
// 删除确认弹层
await card(p, '泡泡').locator('.item').first().locator('.btn.danger:has-text("删除")').click();
await p.waitForTimeout(400);
console.log('确认弹层:', await p.locator('#mTitle').textContent(), '/', (await p.locator('#mBody').textContent()).slice(0, 30) + '…');
await p.click('#mNo');
await p.waitForTimeout(300);
console.log('取消后泡泡数:', await card(p, '泡泡').locator('.item').count());
